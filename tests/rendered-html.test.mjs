...
test("ships colored pixel assets, animated zombies, audio, weapons, and mobile controls", async () => {
  ...
  assert.match(core, /EnemyKind = "thug" \| "runner" \| "brute" \| "thrower" \| "walker" \| "kitty" \| "kittyBoss" \| "neonWarden" \| "ironTitan"/);
  assert.match(core, /enemy\.kind === "thrower" \? "thrown" : enemy\.kind === "neonWarden" \? "neonOrb" : "labubu"/);
  assert.match(core, /state\.wave === 10 \? "neonWarden"/);
  assert.match(core, /state\.wave === 15 \? "ironTitan"/);
  ...
  assert.match(core, /state\.wave === 5 \? "kittyBoss"/);
